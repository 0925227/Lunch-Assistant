...
namespace NeuralNetwork.Helpers
{
	...
	{
		public static Network ImportNetwork(string user)
		{
			var dn = GetHelperNetwork(user);
		...
		public static List<DataSet> ImportDatasets()
		{
			try
			{
                ...
                //var test = System.IO.File.ReadAllLines(@"wwwroot\data\DatasetExample.txt");

                using (var file = File.OpenText(@"wwwroot\data\DatasetExample.txt"))
                {
                    return JsonConvert.DeserializeObject<List<DataSet>>(file.ReadToEnd());
                }
            }
		...
		private static HelperNetwork GetHelperNetwork(string user)
		{
			try
			{
                using (var file = File.OpenText($@"wwwroot\data\{user}.txt"))
                {
